...
def generate_key():
    ...
    block1 = digits[3] + digits[4] + digits[5] +\
          random.choice(string.ascii_uppercase) +\
              random.choice(string.ascii_uppercase)
    block2 = digits[0] + digits[1] + digits[2] +\
          random.choice(string.ascii_uppercase) +\
              random.choice(string.ascii_uppercase)
...
try:
    ...
    messagebox.showerror("Ошибка",\
                          "Фоновое изображение background.png не найдено!")

label_input = tk.Label(root, text="Введите шестизначное число (DEC):",\
...
button_generate = tk.Button(root, text="Сгенерировать ключ",\
                             font=("Arial", 14), command=generate_key,\
                                  bg="green", fg="white")
canvas.create_window(300, 150, window=button_generate)

label_key = tk.Label(root, text="Сгенерированный ключ:",\
                      font=("Arial", 12), bg="lightblue")
